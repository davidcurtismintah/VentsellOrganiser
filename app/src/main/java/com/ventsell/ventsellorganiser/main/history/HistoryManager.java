...
import com.ventsell.ventsellorganiser.main.history.database.DBHelper;
 ...
 * @author Sean Owen
 */
public final class HistoryManager { // TODO use AsyncQueryManager to query database
    ...
    private static final String[] COLUMNS = {
            ...
            DBHelper.INPUT_TYPE_COL,
            DBHelper.RESULT_TYPE_COL
    };
...
//        enableHistory = prefs.getBoolean(PreferencesActivity.KEY_ENABLE_HISTORY, true);
    ...
    public List<HistoryItem> buildHistoryItems() {
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                int inputType = cursor.getInt(4);
                int resultType = cursor.getInt(5);
                Result result = new Result(inputType, text, timestamp, resultType);
    ...
    public void addHistoryItem(Result result) {
        ...
        if (!activity.getIntent().getBooleanExtra(Constants.Scan.SAVE_HISTORY, true) || !enableHistory) {
            return;
        }

        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(activity);
//        if (!prefs.getBoolean(PreferencesActivity.KEY_REMEMBER_DUPLICATES, false)) {
//            deletePrevious(result.ticketId);
//        }
        deletePrevious(result.input.ticketId);

        ContentValues values = new ContentValues();
        values.put(DBHelper.TEXT_COL, result.input.ticketId);
        values.put(DBHelper.DISPLAY_COL, result.getDisplayContents());
        values.put(DBHelper.TIMESTAMP_COL, System.currentTimeMillis());
        values.put(DBHelper.INPUT_TYPE_COL, result.input.inputType);
        values.put(DBHelper.RESULT_TYPE_COL, result.resultType);
    ...
    /**
     * <p>Builds a ticketId representation of the scanning history. Each scan is encoded on one
    ...
    static Uri saveHistory(String history) {
        File bsRoot = new File(Environment.getExternalStorageDirectory(), "VentsellOrganizer");
